...
func (c *Client) Run(ctx context.Context, req *Request, resp interface{}) error {
    if len(req.q) > len("subscription") && req.q[0:len("subscription")] == "subscription" {
        if resp_channel, ok := resp.(chan<-interface{}); ok {
            return c.subscribe(ctx, req, resp_channel)
        } else {
            return errors.New("query type is \"subscription\" but the response inteface is not a channel")
        }
    }
    select {
    case <-ctx.Done():
